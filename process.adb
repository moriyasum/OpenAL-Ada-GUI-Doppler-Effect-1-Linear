...
--  S_X_Pos_Pix := Progress * S_X_Dist_Pix + S_X_Space;
      ...
      OpenAL.Source.Get_Gain (Sound_Source, S_Gain);
      OpenAL.Listener.Get_Gain (L_Gain);
      Put_Line ("Source_Gain=" & Float_t'Image (S_Gain) &
                "  Listener Gain=" & Float_t'Image (L_Gain));
...
------------------
   procedure On_Scale_Volume_Cb
     (Widget : access Gtk.Widget.Gtk_Widget_Record'Class) is
      Scale_Widget : constant Gtk.Scale.Gtk_Hscale :=
        Gtk.Scale.Gtk_Hscale (Widget);
   begin
      Scale_Value := Gtk.Scale.Get_Value (Scale_Widget);
--         Put_Line ("Volume changed to: " & Glib.Gdouble'Image (Scale_Value));
      Audio_Set_Volume (OpenAL.Types.Float_t (Scale_Value));

   end On_Scale_Volume_Cb;
--
--
--
--------------------------------------------------------
