...
--               Integer'Image (WAV_Byte_Length) & " Byte");
      ...
      OpenAL.Buffer.Generate_Buffers (WAV_Buffers);
...
--      Buffer_ID => WAV_Buffers (1),           --  Types.Unsigned_Integer_t;
...
--   );
--   Put_Line ("OpenAL.Thin.Buffer_Data done");

--      OpenAL.Buffer.Buffer_Data
--        (Buffer    => WAV_Buffers (1),
--         Format    => OpenAL.Thin.AL_FORMAT_MONO16,
--         Data      => WAV_File.Data (1)'Address,
--         Size      => OpenAL.Types.Size_t (WAV_Byte_Length),
--         Frequency => OpenAL.Types.Size_t (WAV_File.Header.SampleRate));
...
--  Volume Control
      Audio_Set_Volume (OpenAL.Types.Float_t (Scale_Value));

--  Rolloff Factor, emphasizes approaching and receding
      OpenAL.Source.Set_Rolloff_Factor_Float
        (Sound_Source, Float_t (Source_Gain)); --  Default=1.0,  Sensitive >1.0
--
...
---------------------------------------------------------------------------
--      SetPosF := 100.0;     --  Just for initialize
...
---------------------------------------------------------------------
--  LISTENER
--  alListener3f(AL_POSITION, 0, 0, -2.0f);
...
--
   ...
   begin
         OpenAL.Source.Stop (Sound_Source);
   end Audio_Stop;
--
--
-------------------------------------
--  Set Audio Playing Parameters
-------------------------------------
   procedure Audio_Playing_Parameters is
...
--  Set Velocity
      ...
      --  Just Front then Speed=0
         ...
         --  Moved into Dead zone
            OpenAL.Source.Set_Velocity_Float (Sound_Source, 0.0, 0.0, 0.0);
...
--
      ...
      -------------------------------------
            ...
            begin
               ...
               OpenAL.Source.Set_Gain
                 (Sound_Source, Float_t (Source_Gain * Gain));
               Put_Line
                 ("Deadzone Gain Factor=" & Float'Image (Source_Gain * Gain));
            end;
         --  OpenAL.Source.Set_Direction_Float (Sound_Source, 0.0, 0.0, -1.0);
      ...
      --  Not Deadzone, Regular position (Not just front)
         ...
         OpenAL.Source.Set_Velocity_Float (Sound_Source, SetVelF, 0.0, 0.0);

         if Previous_Dead_Zone then
         --  Just after leaving the Dead Zone, return to the normal output
            OpenAL.Source.Set_Gain (Sound_Source, Float_t (Source_Gain));

            OpenAL.Source.Set_Rolloff_Factor_Float
              (Sound_Source, 1.0);    --  Default=1.0,  Sensitive >1.0
            OpenAL.Global.Set_Doppler_Factor (1.0); --  Default Doppler factor
         end if;
--
      end if;
...
--      if Audio_Dead_Zone then
      --  Rolloff Factor, emphasizes approaching and receding
---         OpenAL.Source.Set_Rolloff_Factor_Float
---           (Sound_Source, 0.0);    --  Default=1.0,  Sensitive >1.0
--  Doppler Factor

--
--      else
--         Audio_Dead_Zone := False;
--         OpenAL.Source.Set_Rolloff_Factor_Float--          (Sound_Source, 1.0);    --  Default=1.0,  Sensitive >1.0
--        OpenAL.Global.Set_Doppler_Factor (1.0);   --  Default Doppler factor
--      end if;

--
--  Set Direction is not needed. Default is Omnidirectional
--      if Source_Direction = Right then
      --  To Right
--         OpenAL.Source.Set_Direction_Float (Sound_Source, 0.0, 0.0, 0.0);
--      else
      --  To Left
--         OpenAL.Source.Set_Direction_Float (Sound_Source, 0.0, 0.0, 0.0);
--      end if;
--
   ...
   end Audio_Playing_Parameters;
--
--
-------------------------------------
--  Set Audio Volume
-------------------------------------
   procedure Audio_Set_Volume (Volume : OpenAL.Types.Float_t) is
   begin
      OpenAL.Source.Set_Gain (Sound_Source, Float_t (Source_Gain));
--
      OpenAL.Listener.Set_Gain (Volume);
   end Audio_Set_Volume;
--
--
----------------------------------------------------------
--  Audio END
